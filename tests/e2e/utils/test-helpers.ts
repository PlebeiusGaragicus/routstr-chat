...
export async function waitForTopupModal(page: Page, timeout = 5000) {
  await page.waitForSelector('text=Top Up', { 
    state: 'visible',
    timeout 
  }).catch(() => {
...
export async function waitForModelsToLoad(page: Page, timeout = 10000) {
  ...
  await page.waitForSelector('#model-selector-drawer', { 
    state: 'visible',
    timeout 
  }).catch(() => {
  ...
  await page.waitForSelector('text=All Models', { 
    state: 'visible',
    timeout 
  }).catch(() => {
...
export async function selectAvailableModel(page: Page): Promise<string | null> {
  ...
  await page.waitForSelector('text=All Models', { timeout: 10000 }).catch(() => {
    // Try alternative: wait for any model item
    return page.waitForSelector('[class*="rounded-md"]', { timeout: 10000 }).catch(() => {});
  });
  ...
  if (allModelsVisible) {
    ...
    const modelItems = allModelsSection.locator('div[class*="rounded-md"]').filter({
 ...
 */
export async function waitForUserMessage(page: Page, messageText: string, timeout = 5000) {
 ...
 */
export async function receiveCashuToken(page: Page, token: string): Promise<boolean> {
  // Wait for topup modal to appear
