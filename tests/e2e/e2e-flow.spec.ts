...
// Test constants
const CASHU_TOKEN = 'cashuBo2FteBxodHRwczovL21pbnQuY3ViYWJpdGNvaW4ub3JnYXVjc2F0YXSBomFpSAC58En8vOXiYXCBo2FhAmFzeEBkMTk1MDQ5MmExMzgwZjcyMGI2Yjk4MzMwOWY5MmFjMzUxYTFlYzVjNjJhMWUzY2QyMWM2MTZkYWU1NTI3ODkyYWNYIQNL_qZWdZGsadUn6jHqIXOwAhz8CmrXo4WFsIdfD-NLmg';
...
test.describe('End-to-End Flow', () => {
  ...
  test('should display connect wallet button in Wallet tab', async ({ page }) => {
    const modalVisible = await ensureModalVisible(page);
    if (modalVisible) {
      await switchToWalletTab(page);
      
      // Check for connect wallet button (may show "Connect Wallet" or "Connected" depending on state)
      const connectButton = page.getByRole('button', { name: /connect wallet|connected/i });
  ...
  test('should enable receive button when token is entered', async ({ page }) => {
    const modalVisible = await ensureModalVisible(page);
    if (modalVisible) {
      await switchToTokenTab(page);
      ...
      const initiallyDisabled = await receiveButton.isDisabled().catch(() => false);
  ...
  test('should paste token from clipboard when clicking paste button', async ({ page, context }) => {
  ...
  test('should attempt to receive token when clicking receive button', async ({ page }) => {
    const modalVisible = await ensureModalVisible(page);
    if (modalVisible) {
      await switchToTokenTab(page);
      ...
      const receivingVisible = await receivingText.isVisible().catch(() => false);
